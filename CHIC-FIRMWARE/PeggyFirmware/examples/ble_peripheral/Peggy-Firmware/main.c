...
#define DEVICE_NAME                      "Peggy"                               			/**< Name of device. Will be included in the advertising data. */
#define MANUFACTURER_NAME                "CHIC TEAM-FR"                      						/**< Manufacturer. Will be passed to Device Information Service. */
...
#define COIN_SENSORS_INTERVAL      			 APP_TIMER_TICKS(5000, APP_TIMER_PRESCALER) 
#define	DEBOUNCE_INTERVAL								 APP_TIMER_TICKS(100, APP_TIMER_PRESCALER) 
...
{
		NRF_GPIO->OUTCLR = (BSP_LED_2_MASK | BSP_LED_3_MASK | BSP_LED_4_MASK | BSP_LED_5_MASK | BSP_LED_6_MASK);
		NRF_GPIO->OUTSET = BSP_LED_1_MASK;
	
		switch(current_sensor)
		{
			case 1:							// first Sensor
				test_function(4);	// -.50 CHF
				break;
			case 2:							// 2nd sensor
				test_function(1);	// -.10 CHF
				break;
			case 3:							// 3rd sensor
				test_function(2);	// -.20 CHF
				break;
			case 4:							// 4th sensor
				test_function(8);	// 1.- CHF
				break;
			case 5:							// 5th sensor
				test_function(16);// 2.- CHF
				break;
			case 0:							// 6th sensor
				test_function(32);// 5.- CHF
				break;
			default:
				break;
		}
...
static void application_timers_start(void)
{
    ...
    uint32_t err_code;
    //err_code = app_timer_start(m_sensor_timer_id, COIN_SENSORS_INTERVAL, NULL);
    APP_ERROR_CHECK(err_code);
}
...
// A3 LPCOMP config
static nrf_drv_lpcomp_config_t m_lpcomp_AIN3 = {{NRF_LPCOMP_REF_SUPPLY_5_8 , NRF_LPCOMP_DETECT_UP},
																								NRF_LPCOMP_INPUT_3,
																								APP_IRQ_PRIORITY_HIGHEST};

// A3 event handler
static void A3_event_handler(nrf_lpcomp_event_t event){
		//LEDS_INVERT(BSP_LED_0_MASK); // just change state of first LED
		nrf_drv_lpcomp_disable();
		app_timer_start(m_debounce_timer_id,
										DEBOUNCE_INTERVAL,
										NULL);
		...
		switch(current_sensor){
			case 0:
				...
				NRF_GPIO->OUTSET = BSP_LED_2_MASK;
				current_sensor = 1;
				app_timer_start(m_sensor_timer_id,
										COIN_SENSORS_INTERVAL,
										NULL);
				break;
			case 1:
				NRF_GPIO->OUTCLR = BSP_LED_2_MASK;
				NRF_GPIO->OUTSET = BSP_LED_3_MASK;
				current_sensor = 2;
				break;
			case 2:
				NRF_GPIO->OUTCLR = BSP_LED_3_MASK;
				NRF_GPIO->OUTSET = BSP_LED_4_MASK;
				current_sensor = 3;
				break;
			case 3:
				NRF_GPIO->OUTCLR = BSP_LED_4_MASK;
				NRF_GPIO->OUTSET = BSP_LED_5_MASK;
				current_sensor = 4;
				break;
			case 4:
				NRF_GPIO->OUTCLR = BSP_LED_5_MASK;
				NRF_GPIO->OUTSET = BSP_LED_6_MASK;
				current_sensor = 5;
				break;
			case 5:
				NRF_GPIO->OUTCLR = BSP_LED_6_MASK;
				NRF_GPIO->OUTSET = BSP_LED_1_MASK;
				current_sensor = 0;
				break;
			default:
...
static void lpcomp_init(void)
{
    ...
    //err_code = nrf_drv_lpcomp_init(NULL, lpcomp_event_handler);
		err_code = nrf_drv_lpcomp_init(&m_lpcomp_AIN3, A3_event_handler);
    APP_ERROR_CHECK(err_code);
    nrf_drv_lpcomp_enable();
}
...
void sensor_decode()
{
		/*if(current_sensor == 0){
				...
				NRF_GPIO->OUTSET = BSP_LED_2_MASK;
				current_sensor = 1;
				app_timer_start(m_sensor_timer_id, 20000, NULL);
		}
...
static void device_manager_init(bool erase_bonds)
{
    ...
    register_param.service_type           = DM_PROTOCOL_CNTXT_GATT_SRVR_ID;
		...
		//pass_dm_info(&m_app_handle, &register_param); 
...
int main(void)
{
    ...
    APP_ERROR_CHECK(err_code);
		...
		//Start Sensor 1
		//NRF_GPIO->OUTSET = BSP_LED_1_MASK;
		//NRF_GPIO->OUTSET = BSP_LED_10_MASK;
