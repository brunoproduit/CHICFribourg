...
{
    uint8_t len = 0; 
    //len += bds_UNDEFINED_TYPE_encode(&p_erase_bonding->killcode, &encoded_buffer[len]); 
    return len;
}
...
{
    uint8_t pos = 0;
    //pos += bds_UNDEFINED_TYPE_decode((data_len-pos), &p_data[pos], &p_write_val->killcode); 
...
{
    uint8_t len = 0; 
    //len += bds_boolean_encode(&p_user_information->balance_ok, &encoded_buffer[len]); 
    len += bds_uint16_encode(&p_user_information->user, &encoded_buffer[len]); 
    return len;
}
...
{
    uint8_t pos = 0;
    //pos += bds_boolean_decode((data_len-pos), &p_data[pos], &p_write_val->balance_ok); 
    pos += bds_uint16_decode((data_len-pos), &p_data[pos], &p_write_val->user); 
